...
class Beyond20 {
    ...
    static async createActorData(request) {
        ...
        // Create a spell level of `true` because of https://gitlab.com/foundrynet/dnd5e_paw/-/issues/1025
        actorData.spells[true] = {value: 1, max: 1, override: 1};

        // Cache SRD classes compendium
        if (!this._srdClasses) {
            const compendium = game.packs.get("dnd5e_paw.classes")
    ...
    static _fillSpellData(request, itemData) {
        ...
        for (const [school, name] of Object.entries(CONFIG.dnd5e_paw.spellSchools)) {
    ...
    static _buildAttackData(request, itemData) {
        ...
        if (request.damages) {
            ...
            for (let i = 0; i < request.damages.length; i++) {
                ...
                if (CONFIG.dnd5e_paw.damageTypes[type.toLowerCase()]) {
    ...
    static async rollSkill(request) {
        ...
        mergeObject(rollOptions, {
            parts: parts,
            data: data,
            title: game.i18n.format("dnd5e_paw.SkillPromptTitle", {skill: CONFIG.dnd5e_paw.skills[skill]}),
            messageData: {"flags.dnd5e_paw.roll": {type: "skill", skill }}
        });
        rollOptions.speaker = ChatMessage.getSpeaker({actor: actor, token: token});
        game.dnd5e_paw.dice.d20Roll(rollOptions);
    }
    ...
    static async rollSavingThrow(request) {
        ...
        mergeObject(rollOptions, {
            parts: parts,
            data: data,
            title: game.i18n.format("dnd5e_paw.SavePromptTitle", {ability: CONFIG.dnd5e_paw.abilities[abl]}),
            messageData: {"flags.dnd5e_paw.roll": {type: "save", abl }}
        });
        rollOptions.speaker = ChatMessage.getSpeaker({actor: actor, token: token});
        game.dnd5e_paw.dice.d20Roll(rollOptions);
    }
    ...
    static async rollAbility(request) {
        ...
        mergeObject(rollOptions, {
            parts: parts,
            data: data,
            title: game.i18n.format("dnd5e_paw.AbilityPromptTitle", {ability: CONFIG.dnd5e_paw.abilities[abl]}),
            messageData: {"flags.dnd5e_paw.roll": {type: "ability", abl }}
        });
        rollOptions.speaker = ChatMessage.getSpeaker({actor: actor, token: token});
        game.dnd5e_paw.dice.d20Roll(rollOptions);
    }
    ...
    static async rollItems(request) {
        ...
        const template = game.dnd5e_paw.canvas.AbilityTemplate.fromItem(actorItem);
